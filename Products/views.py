...
def PrintSellerInvoicesDetails(request, pk):
    ...
    seller_payments_from = SellerPayments.objects.filter(seller=seller, paid_type=1)
    seller_payments_to = SellerPayments.objects.filter(seller=seller, paid_type=2)
    if seller:
